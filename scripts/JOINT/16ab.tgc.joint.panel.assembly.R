...
# Step 16ab: Multi-panel figure assembly
...
#
# INPUTS  (from Step 14ab: 8 individual TIFF plots + legend)
#   RESULTS/JOINT/COMBINED5/manhattan/manhattan_circular_*.tiff
#   RESULTS/JOINT/COMBINED5/manhattan/legend_circular_5.tiff
#
# OUTPUTS
...
############################################################
# 1) PATHS + SOURCE 14ab.tgc.joint.manhattan.plots.R
...
log_msg("Step 16ab panel assembly -- building chromosome layout...")
...
PNL_W_CM  <- OUT_W_CM          # 18 cm (from 14ab script)
...
draw_combined_panel <- function(tool, out_base, fdr_mode) {
  ...
  if (HAS_MAGICK) {
    ...
    if (all(file.exists(img_b, img_n, leg_tif))) {
      TMP <- file.path(PANEL_DIR, ".tmp16combined")
...
log_msg(paste(rep("=", 65), collapse = ""))
log_msg("Step 16ab panel assembly finished")
